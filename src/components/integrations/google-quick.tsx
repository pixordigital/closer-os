...
export function GoogleQuick(){
  ...
  useEffect(()=>{ fetch("/api/integrations/google/check").then(r=>r.json()).then(j=>setGlobalOk(j.configured)).catch(()=>setGlobalOk(false)); },[]);
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      ...
      <p className="text-xs text-zinc-500 mt-1">Conecta OAuth uma vez, usa em Tasks, Follow-ups e Health.</p>
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3">
          ...
          <p className="text-xs text-zinc-500 mt-1">Envio de Follow-up + leitura Inbox</p>
      ...
      {globalOk===true && <div className="mt-4 rounded border border-emerald-900 bg-emerald-950/20 p-3 text-xs text-emerald-300">✓ OAuth global configurado — closer só clica Conectar, sem JSON.</div>}
      {globalOk===false && (
        <div className="mt-4 rounded border border-dashed border-zinc-700 p-3">
          <div className="text-xs font-medium text-zinc-300">Admin: cole o JSON do Google Cloud uma vez (global)</div>
          ...
          <button onClick={saveCreds} disabled={saving || !json.trim()} className="mt-2 rounded bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 hover:bg-zinc-700 disabled:opacity-50">{saving?"Salvando...":"Salvar credenciais (1x global)"}</button>
          {msg && <p className="mt-1 text-xs text-amber-400">{msg}</p>}
        </div>
      )}
      <p className="mt-3 text-[11px] text-zinc-500">Depois, clique Conectar. Callback <code>/api/calendar/callback</code>.</p>
    </div>
  );
}
