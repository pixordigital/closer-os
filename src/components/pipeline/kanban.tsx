        ...
        {byStage.map(col => {
          ...
          return (
              ...
              <div className="flex flex-1 flex-col gap-2 p-2 min-h-[120px]">
                {col.items.length===0 && <div className={`rounded border border-dashed p-4 text-center text-xs ${overStage===col.stage ? "border-sky-600 text-sky-400 bg-sky-950/20" : "border-zinc-800 text-zinc-600"}`}>solte aqui</div>}
                {col.items.map(d=>(
                  <div
                    key={d.id}
                    draggable
                    ...
                    className={`rounded-md border bg-zinc-950 p-3 cursor-grab active:cursor-grabbing select-none transition ${dragId===d.id ? "opacity-40 scale-[0.98] border-sky-700" : "border-zinc-800 hover:border-zinc-700 hover:shadow-lg"}`}>
                    <div className="mb-1 flex items-center gap-1 text-[10px] tracking-widest text-zinc-600">⋮⋮ ARRASTE</div>
