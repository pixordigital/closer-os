  ...
  return (
    <div className="p-6 sm:p-8 space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5"><p className="text-xs uppercase tracking-wide text-zinc-500">{tr("dashboard.pipeline")}</p><p className="mt-1 text-2xl font-semibold">{fmt(pipelineValue,"BRL",locale)}</p><p className="text-xs text-zinc-500">{dealsTotal} deals</p></div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5"><p className="text-xs uppercase tracking-wide text-zinc-500">{tr("dashboard.companies")}</p><p className="mt-1 text-2xl font-semibold">{companies}</p><p className="text-xs text-zinc-500">{locale==="en"?"active accounts":"contas ativas"}</p></div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5"><p className="text-xs uppercase tracking-wide text-zinc-500">{tr("dashboard.calls")}</p><p className="mt-1 text-2xl font-semibold">{callsTotal}</p><p className="text-xs text-zinc-500">{locale==="en"?"total recorded":"total gravadas"}</p></div>
        <div className={`rounded-xl border p-5 ${tasksOverdue>0?"border-amber-900/50 bg-amber-950/20":"border-zinc-800 bg-zinc-900"}`}><p className="text-xs uppercase tracking-wide text-zinc-500">{tr("dashboard.tasks")}</p><p className="mt-1 text-2xl font-semibold">{tasksTodo} <span className="text-sm font-normal text-zinc-500">{tr("dashboard.pending")}</span></p><p className="text-xs text-amber-400">{tasksOverdue>0?`${tasksOverdue} ${tr("dashboard.overdue")}`: tr("dashboard.onTime")}</p></div>
      </div>
