        ...
        <div className="space-y-6">
          <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
            <div className="flex items-center justify-between">
              ...
              <Link href={`/calls/new?dealId=${deal.id}`} className="text-xs text-sky-400 hover:underline">+ Nova call</Link>
            </div>
            <div className="mt-3 space-y-2">
              {deal.calls.length===0 && <p className="text-sm text-zinc-500">Nenhuma call vinculada.</p>}
              {deal.calls.map(c=>(
                <Link key={c.id} href={`/calls/${c.id}`} className="block rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 hover:border-zinc-700">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium text-zinc-100">{c.title}</span>
                    {c.transcript ? <span className="text-[11px] text-emerald-400">transcript ✓</span> : <span className="text-[11px] text-zinc-500">sem transcript</span>}
                  </div>
